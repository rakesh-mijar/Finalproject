...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($First) && !empty($Last) && !empty($USN) && !empty($Email) && !empty($Mobile) && !empty($Gender) && !empty($SelectyourBranch)) {
        ...
        $query = "INSERT INTO `captcha3` (`FirstName`, `LastName`, `USN`, `Email`, `Mobile`, `Gender`, `Branch`, `Semester`) VALUES ('$First', ' $Last', '$USN', '$Email', '$Mobile', '$Gender', '$SelectyourBranch', '$Selectcurrentsemester')";
        if ($query) {
    ...
    <div class="container">
        <div class="row register">
            ...
            <div class="col-sm-6 reg">
                <form class="form-horizontal hr" method="post" action="captcha.php">
